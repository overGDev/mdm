...
mod tests {
    ...
    #[test]
    fn header_id_matches_autogenerated_format() {
        let cases = [
            (
            ...
            (
                ...
                "52-landing-page-services--applications-implementation",
            ),
        ];
        ...
        for (title, expected_id) in cases {
            ...
            let expected_index = format!("- [{}](#{})", title, expected_id);
            assert_eq!(node.get_section_index(1), expected_index, "failed for: {}", title);
        }
    }
